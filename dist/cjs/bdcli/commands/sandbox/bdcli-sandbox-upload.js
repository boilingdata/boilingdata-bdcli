...
async function show(options, _command) {
    try {
        ...
        (0, spinner_util_js_1.updateSpinnerText)("Uploading sandbox IaC template");
...
const program = new cmd.Command("bdcli sandbox upload")
    ...
    .addOption(new cmd.Option("--allow-changed-filename", "Allow local template filename chnage with an existing template ID (possible copy/paste error)"))
